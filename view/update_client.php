...
<h1>Modifier un client</h1>
    <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
        <input name="id" type="number" value="<?php echo (isset($_POST['id'])) ? $_POST['id'] : $id;?>">
        <div><label for="lastName">Nom:</label>
        <input type="text" id="lastName" name="lastName" value="<?php echo htmlspecialchars($_SESSION['modifying'][$id-1]['lastName']);?>"></div>
        <div><label for="firstName">Prénom:</label>
        <input type="text" id="firstName" name="firstName" value="<?php echo htmlspecialchars($_SESSION['modifying'][$id-1]['firstName']);?>"></div>
        <div><label for="birthDate">Date de naissance:</label>
        <input type="date" id="birthDate" name="birthDate" value="<?php echo htmlspecialchars($_SESSION['modifying'][$id-1]['birthDate']);?>"></div>
        <div><label for="fidelityCard">Carte de fidélité:</label>
        <p><label for="oui">Oui</label>
        <input type="radio" name="fidelityCard" id="oui" value="true" required <?php echo ($_SESSION['modifying'][$id-1]['card'] == 1) ? 'checked' : ''?>></p>
        <p><label for="non">Non</label>
        <input type="radio" name="fidelityCard" id="non" value="false" <?php echo ($_SESSION['modifying'][$id-1]['card'] == 0) ? 'checked' : ''?>></div></p></div>

        <div><label for="cardNumber">Numéro de carte:</label>
        <input type="text" name="cardNumber" id="cardNumber" value="<?php echo htmlspecialchars($_SESSION['modifying'][$id-1]['cardNumber']);?>"></div>
